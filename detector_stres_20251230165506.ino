...
void loop() {
  ...
  // ===== TELEMETRY PAYLOAD PENTRU TESTARE AUTOMATA (HiL) =====
    ...
    if (millis() - lastTelemetry >= 1000) {
      ...
      // Format industrial CSV: TIMESTAMP, BPM, TEMP, PIEZO_ENV, GSR_SCORE, TOTAL_SCORE, SYSTEM_STATE
      Serial.print(millis());        Serial.print(",");
      Serial.print(bpmAvg, 1);       Serial.print(",");
      Serial.print(tempC, 2);        Serial.print(",");
      Serial.print(piezoEnvelope);   Serial.print(",");
      Serial.print(gsrLastScore);    Serial.print(",");
      Serial.print(lieScore);        Serial.print(",");
      
      // Sending the state of the system  (0=Init, 1=Calibration, 2=Ready)
      int sysState = 2; 
      if (!fingerDetected) sysState = 0;
      else if (!gsrCalDone) sysState = 1;
